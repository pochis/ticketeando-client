    ...
    <v-card-title>
      ...
      <v-data-table
        ...
        <template slot="items" slot-scope="props">
          <td><nuxt-link :to="{name: 'api-show-id', params: { id: props.item.id }}">{{ props.item.id }}</nuxt-link></td>
          ...
          <v-btn icon class="mx-0" :to="{name: 'api-show-id', params: { id: props.item.id }}">
            <v-icon color="teal">edit</v-icon>
          </v-btn>
          <v-btn v-if="props.item.id!=1" icon class="mx-0" @click="deleteItem(props.item)">
...
<script>
    export default {
        data:()=>({
          ...
          titles: [
                { text: 'ID', value: 'id' },
                { text: 'API Key',align: 'left',value: 'secret'},
                { text: 'Dominio', value: 'domain' },
                { text: 'Correo responsable', value: 'email' },
        ...
        methods: {
          getData () {
            ...
            let endpoint='api/list/'+this.offset+"/"+this.limit;
          ...
          deleteItem(item){
            ...
            if(conf){
              this.loading =true;
              this.$axios.delete('api/'+item.id,this.headers).then((res)=>{
